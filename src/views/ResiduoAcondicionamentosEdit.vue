...
export default {
  ...
  methods: {
    ...
    async loadEntity() {
      ...
      let result = await residuoAcondicionamentosController.get(this.$route.params.id)
      ...
      if (!result.error){
        ...
        this.$router.push({ path: "/residuo-acondicionamentos" });
      }
    },
    ...
    async save() {
      if (this.valid) {
        ...
        let residuoAcondicionamentosController = new ResiduoAcondicionamentosController();
        let result = await residuoAcondicionamentosController.update(this.formFields);
  ...
  mounted() {
    this.SET_TOOLBAR_BACK_URL('/residuo-acondicionamentos')
    this.loadEntity()
  }
};
</script>