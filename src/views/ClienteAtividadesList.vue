        ...
        >
          <v-card-title>
            <v-btn
              color="primary"
              large
              to="cliente-atividades/adicionar"
            >
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      let result = await clienteAtividadesController.list(
        filters,
        pagination.page,
    ...
    async onDeleteItem(item) {
      this.SHOW_LOADER()
      let clienteAtividadesController = new ClienteAtividadesController();
      let result = await clienteAtividadesController.delete(item);
      this.CLOSE_LOADER()
    ...
    onEditItem(item) {
      this.$router.push({ path: `/cliente-atividades/editar/${item}` });
    }
  },
