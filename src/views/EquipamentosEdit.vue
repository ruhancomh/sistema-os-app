...
export default {
  ...
  methods: {
    ...
    async loadEntity() {
      ...
      let result = await equipamentosController.get(this.$route.params.id)
      ...
      if (!result.error){
        ...
        this.$router.push({ path: "/equipamentos" });
      }
    },
    ...
    async save() {
      if (this.valid) {
        ...
        let equipamentosController = new EquipamentosController();
        let result = await equipamentosController.update(this.formFields);
  ...
  mounted() {
    this.SET_TOOLBAR_BACK_URL('/equipamentos')
    this.loadEntity()
  }
};
</script>