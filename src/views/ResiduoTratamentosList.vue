        ...
        >
          <v-card-title>
            <v-btn
              color="primary"
              large
              to="residuo-tratamentos/adicionar"
            >
              <v-icon dark>add</v-icon>
              Adicionar tratamento de residuo
            </v-btn>
          </v-card-title>
          <v-divider />
          <v-card-text>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      let result = await residuoTratamentosController.list(
        filters,
        pagination.page,
    ...
    async onDeleteItem(item) {
      this.SHOW_LOADER()
      let residuoTratamentosController = new ResiduoTratamentosController();
      let result = await residuoTratamentosController.delete(item);
      this.CLOSE_LOADER()
    ...
    onEditItem(item) {
      this.$router.push({ path: `/residuo-tratamentos/editar/${item}` });
    }
  },
