...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      let result = await equipamentosController.list(
        filters,
        pagination.page,
    ...
    async onDeleteItem(item) {
      this.SHOW_LOADER()
      let equipamentosController = new EquipamentosController();
      let result = await equipamentosController.delete(item);
      this.CLOSE_LOADER()
    ...
    onEditItem(item) {
      this.$router.push({ path: `/equipamentos/editar/${item}` });
    }
  },
