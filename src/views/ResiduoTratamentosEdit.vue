...
export default {
  ...
  methods: {
    ...
    async loadEntity() {
      ...
      let result = await residuoTratamentosController.get(this.$route.params.id)
      ...
      if (!result.error){
        ...
        this.$router.push({ path: "/residuo-tratamentos" });
      }
    },
    ...
    async save() {
      if (this.valid) {
        ...
        let residuoTratamentosController = new ResiduoTratamentosController();
        let result = await residuoTratamentosController.update(this.formFields);
  ...
  mounted() {
    this.SET_TOOLBAR_BACK_URL('/residuo-tratamentos')
    this.loadEntity()
  }
};
</script>