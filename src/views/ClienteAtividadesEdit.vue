...
export default {
  ...
  methods: {
    ...
    async loadEntity() {
      ...
      let result = await clienteAtividadesController.get(this.$route.params.id)
    ...
    async save() {
      if (this.valid) {
        ...
        let clienteAtividadesController = new ClienteAtividadesController();
        let result = await clienteAtividadesController.update(this.formFields);
  ...
  mounted() {
    this.SET_TOOLBAR_BACK_URL('/cliente-atividades')
    this.loadEntity()
  }
};
</script>