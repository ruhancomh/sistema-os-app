...
export default {
  ...
  methods: {
    ...
    async save() {
      if (this.valid) {
        ...
        let result = await manifestosController.create(this.formFields);
        ...
        if (!result.error)
          this.$router.push({ path: `/manifestos/editar/${result.data.id}` });
  ...
  async created() {
    ...
    this.formFields = new ManifestosController().getModel()
