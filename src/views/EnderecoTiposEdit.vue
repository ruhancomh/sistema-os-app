...
export default {
  ...
  methods: {
    ...
    async loadEntity() {
      ...
      let result = await enderecoTiposController.get(this.$route.params.id)
      ...
      if (!result.error){
        ...
        this.$router.push({ path: "/endereco-tipos" });
      }
    },
    ...
    async save() {
      if (this.valid) {
        ...
        let enderecoTiposController = new EnderecoTiposController();
        let result = await enderecoTiposController.update(this.formFields);
  ...
  mounted() {
    this.SET_TOOLBAR_BACK_URL('/endereco-tipos')
    this.loadEntity()
  }
};
</script>